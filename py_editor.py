...
class TEXT_HT_header(Header):
	...
	def draw(self, context):
		...
		if text:
			...
			row = layout.row()
			row.operator("text.insert_comment_block")
...
'''
# Doesn't work (cannot find end line index)
def comment_selected_code(code):
	start_line = code.current_line
	end_line = code.select_end_line
	
	start_line['selected'] = end_line['selected'] = True
	do_comment = add_comment = False
	
	for line in code.lines:
		if 'selected' in line:
			add_comment = not is_commented_line(line)
			comment_line(line) if add_comment else uncomment_line(line)
			do_comment = not do_comment # if 'selected' in line else do_comment
		elif do_comment:
			comment_line(line) if add_comment else uncomment_line(line)
'''
...
class TEXT_MT_format(Menu):
	...
	def draw(self, context):
		...
		layout.operator("text.comment_line")
